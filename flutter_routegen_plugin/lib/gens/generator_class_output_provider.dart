...
class GeneratorClassOutputProvider {
  ...
  var constantsForRoute = Set<String>();
  ...
  String moduleImports(){
    modules.forEach((package,module) {
      module.differentClassNameImport =
          ModuleSplitter.convertToCamelCase(snakeCase, module.modulePackage);
      headers.add(
          "import 'package:${module.originalPath}' as ${module.differentClassNameImport};");
    });
  }
  ...
  String get allConstants {
    ...
    constantsForRoute.forEach((method) {
      constantBuffer.writeln(method);
    });
  ...
  String get moduleVariables {
    ...
    modules.forEach((package,module) {
      var variableName = module.className.substring(0, 1).toLowerCase() +
          module.className.substring(1);
  ...
  String generateHandler() {
    var buffer = StringBuffer();
    buffer.writeln(
        "${getTabs(1)}static Route<dynamic> generateRoutes(RouteSettings routeSettings){");

    if(modules.length>0){
      modules.forEach((package,module){
        var moduleFoundName = "${variableNameTransformation(ModuleSplitter.convertToCamelCase(snakeCase, package))}RouteFound";
        buffer.writeln("${getTabs(2)}var $moduleFoundName = ${getModuleClassName(module)}.generateRoutes(routeSettings);");
    ...
    mapOfRouteAndClass.forEach((key, value) {
      buffer.writeln("${getTabs(3)}case ROUTE_NAME_${key.toUpperCase()}:");
      buffer.writeln(
          "${getTabs(4)}openScreen = ${value.routeOrigin}(${value.methodParamString});");
  ...
  generateForAnnotatedElement(
    ...
    addConstants(paramObjectType, routeName);

    var routeContainer = RouteContainer(
        params: paramObjectType,
        routeName: "ROUTE_NAME_${routeName.toUpperCase()}",
        routeOrigin: element.displayName,
        methodParamString:
            generateRouteChecksForConstructors(visitor, paramObjectType));
    mapOfRouteAndClass[routeName] = routeContainer;

    methods.add(addRoutingMethodInList(routeName, paramsForScreen));
  }

  generateRouteChecksForConstructors(
      ClassVisitor visitor, DartType paramObjectType) {
    ...
    visitor.constructorParameters.forEach((parameters) {
      if (paramObjectType == parameters.type) {
        ...
        if (parameters.isOptional)
          method.write(
              "${parameters.name}: routeSettings.arguments as $paramObjectType");
        else
    ...
    if (!isParameterFound) {
      errors.add(
          "Please add $paramObjectType as parameter for ${visitor.className}");
      throwException(
          "Please add $paramObjectType as parameter for ${visitor.className}");
    }
  ...
  addRoutingMethodInList(String routeName, String paramsForScreen) {
    StringBuffer stringBuffer = StringBuffer();
    stringBuffer.writeln(
        "${getTabs(1)}open${capitalize(routeName)}({@required BuildContext context, $paramsForScreen}){");
    stringBuffer.writeln(
  ...
  parseHeaders(Element element, DartType paramObjectType) {
    ...
    if (paramObjectType != null) {
      ...
      if (!checkIfPrimaryType(paramObjectType))
        headers
            .add("import '${paramObjectType.element.source.uri.toString()}';");
      paramsForScreen = "@required $paramObjectType params";
    }
    return paramsForScreen;
  }

  /// addConstants method will take [paramObjectType], [routeName] for creating constants of routeNames
  addConstants(DartType paramObjectType, String routeName) {
    if (constantsForRoute.contains(routeName)) {
      errors.add(
          "The route $routeName you have defiend for $paramObjectType has been already used by another class");
      throwException(
          "The route $routeName you have defiend for $paramObjectType has been already used by another class");
    }
    constantsForRoute
        .add("const ROUTE_NAME_${routeName.toUpperCase()} = \"$routeName\";");
  }
...
class ModuleSplitter {
    ...
    try {
      ...
      while (firstMatch != null) {
        newValue = newValue.substring(0, firstMatch.start) +
            newValue
                .substring(firstMatch.start + 1, firstMatch.start + 2)
                .toUpperCase() +
  ...
  static String convertToSnakeCase(String value) {
    try {
      ...
      var firstMatch = regExp.firstMatch(newValue);
      while (firstMatch != null) {
        newValue = newValue.substring(0, firstMatch.start) +"_"+
            newValue
                .substring(firstMatch.start, firstMatch.start+1)
                .toLowerCase() +
